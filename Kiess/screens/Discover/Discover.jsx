...
const Chat = require("../../assets/Discorver/Chat.png");
const PieceGold = require("../../assets/Discorver/pieceGold.png");
const PieceRouge = require("../../assets/Discorver/pieceRouge.png");
...
export default function Discover({ route, navigation }) {
  ...
  const badgeRows = Array.from(
    { length: Math.ceil(badges.length / 4) },
    (_, i) => badges.slice(i * 4, (i + 1) * 4)
  );
  ...
  <View style={tw`items-center`}>
    ...
    <LinearGradient
        // Les styles du gradient
        colors={['#FFFFFF', '#FC2D3D']}
        style={tw`h-22 w-22 justify-center items-center rounded-lg`}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
      >
        <Image source={Chat} style={tw`absolute bottom-15 w-10 h-10`} resizeMode="contain" />
        <Text style={tw`text-lg text-red-500 pt-10`}>0</Text>
        <Text style={[tw`text-gray-500 font-semibold`, { fontSize: 10 }]}>SUPER PIÈCES(S)</Text>
      </LinearGradient>
  </View>
  ...
  <LinearGradient
      ...
      <Image source={PieceGold} style={tw`absolute bottom-15 w-10 h-10`} resizeMode="contain" />
      <Text style={tw`text-lg text-yellow-500 pt-10`}>{points}</Text> 
      <Text style={[tw`text-gray-500 font-semibold`, { fontSize: 10 }]}>POINTS(S)</Text>
      </LinearGradient>
  ...
  <LinearGradient
      ...
      >
      <Image source={PieceRouge} style={tw`absolute bottom-15 w-9 h-9`} resizeMode="contain" />
      <Text style={tw`text-lg text-red-500 pt-10`}>0</Text> 
      <Text style={[tw`text-gray-500 font-semibold`, { fontSize: 10 }]}>SUPER PIÈCES(S)</Text>
      </LinearGradient>
